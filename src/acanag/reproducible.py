    ...
    if np.shape(X_curr)[0] >= n_data_min:
        ...
        if len(Y_lab) >= min_n_labeled and sum([1 for i in range(len(Y_lab)) if Y_lab[i]==0]) >= min_n_nom and sum([1 for i in range(len(Y_lab)) if Y_lab[i]==1]) >= min_n_anom:
            ...
            if supervised_method == 'RandomForestClassifier':
                ...
                weights = {0: 1-r0, 1: r0}
                RFC = RandomForestClassifier(class_weight=weights,max_depth = np.ceil(5*np.sqrt(np.shape(X_new)[1])).astype(int))
        ...
        if len(Y_lab) > min_n_labeled and sum([1 for i in range(len(Y_lab)) if Y_lab[i]==0]) >= min_n_nom and sum([1 for i in range(len(Y_lab)) if Y_lab[i]==1]) >= min_n_anom:
            ...
            if supervised_method == 'RandomForestClassifier':
                ...
                estimator=RandomForestClassifier(class_weight=weights,max_depth = np.ceil(5*np.sqrt(np.shape(X_new)[1])).astype(int)),
...
def ActiveAGG_3(X_new = None, X_old = None, X_lab = None, Y_lab = None, all_labeled_scores = None, models=None,supervised_method = None,n_data_min = 100,n_data_max = 2000,min_n_labeled = 5,n_send=2,pc_top = 0.5,min_n_nom=5,min_n_anom=1,tau_exp=0.01,curr_method = None,**kwargs):
    ...
    # new data time point, it uses active learning to propose some of these to an expert, and it
    # predicts new anomalies using the current version of the supervised machine learning model.
    # The difference with ActiveAGG_2 is exporting an extra variables.
    #
    # Arguments: 
    #    X_new : new numpy array batch of B time-series data of size B x number of data dimensions
    #    X_old : numpy array of all (or most recent) past data
    ...
    if np.shape(X_curr)[0] >= n_data_min:
        ...
        if len(Y_lab) >= min_n_labeled and sum([1 for i in range(len(Y_lab)) if Y_lab[i]==0]) >= min_n_nom and sum([1 for i in range(len(Y_lab)) if Y_lab[i]==1]) >= min_n_anom:
            ...
            if supervised_method == 'RandomForestClassifier':
                ...
                r0 = 1-tau_exp  # Proportion of class 0 expected
                weights = {0: 1 - r0, 1: r0}
                RFC = RandomForestClassifier(class_weight=weights,max_depth=10)
                learned_model = RFC.fit(all_labeled_scores, Y_lab)
                # Predicted probabilities for the positive class
                new_preds = learned_model.predict_proba(all_scores)[:, 1]
                #print('new_preds',new_preds)
        ...
        if len(Y_lab) > min_n_labeled and sum([1 for i in range(len(Y_lab)) if Y_lab[i]==0]) >= min_n_nom and sum([1 for i in range(len(Y_lab)) if Y_lab[i]==1]) >= min_n_anom:
            ...
            if supervised_method == 'RandomForestClassifier':
                ...
                learner = ActiveLearner(
                #estimator=RandomForestClassifier(class_weight='balanced'),
                estimator=RandomForestClassifier(class_weight=weights,max_depth=10),
                #query_strategy=uncertainty_sampling,
                query_strategy = margin_sampling,
                #query_strategy = entropy_sampling,
                X_training=curr_all_labeled_scores, y_training=curr_Y_lab
    ...
    return X_old, X_lab, all_labeled_scores, indices_to_expert, learned_model, supervised_indices, new_preds 
